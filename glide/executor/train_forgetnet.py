"""
glide/executor/train_forgetnet.py
...
graph's rollout (per-graph reset), so the processor can remember past states.
If this gated-memory model does NOT beat the history-free EquivariantExecutor,
that strengthens the Markov-operator interpretation (X_{t+1}=Phi(G,X_t,tau_t)
is Markov by construction; memory is unnecessary).

Usage:
    .venv/bin/python -m glide.executor.train_forgetnet \
        --dataset_path data/processed/comm_5k_v2_with_encodings.pt \
        --output_dir checkpoints/forgetnet_v1 --num_epochs 50 --device cuda:0
"""
...
def graph_loss(encoder, model, data, device):
    ...
    total = 0.0
    n_steps = 0
    h_state = None  # carried across steps, reset per graph
    for step in iter_steps(data):
        pos_before = step.pos_before.to(device)
        pos_after = step.pos_after.to(device)
        tau = step.tau.to(device)

        dX_pred, h_state = model(node_feat, edge_index, pos_before, tau, h_state=h_state)
...
def main():
    ...
    model = GForgetNet(node_feat_dim=args.hidden_dim, hidden_dim=args.hidden_dim,
