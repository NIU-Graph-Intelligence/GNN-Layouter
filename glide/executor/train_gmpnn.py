...
TripletGMPNN -- the strongest standard NAR processor. The comparison against
phi1_v1 (EquivariantExecutor) answers "we did not win because we used a
stronger architecture": same supervision, stronger processor, only alignment
differs.

Usage:
    .venv/bin/python -m glide.executor.train_gmpnn \
        --dataset_path data/processed/comm_5k_v2_with_encodings.pt \
        --output_dir checkpoints/gmpnn_v1 --num_epochs 50 --device cuda:0
"""
...
def main():
    ...
    model = TripletGMPNN(node_feat_dim=args.hidden_dim, hidden_dim=args.hidden_dim,
