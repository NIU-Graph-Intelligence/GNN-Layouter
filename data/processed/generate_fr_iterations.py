...
Runs FR layout step-by-step and saves every intermediate state as a
training sample for single-step white-box learning.

Output schema (pickle):
    {
      'graphs': [
        {
          'graph_idx': int,
          ...
          'steps': [
            {
              'step':        int,
              'pos_before':  np.ndarray [N, 2],
              'pos_after':   np.ndarray [N, 2],
              'temperature': float,
            },
            ...
          ]
...
# ─────────────────────────────────────────────────────────────────────────────
# FR single step  (mirrors NetworkX _fruchterman_reingold exactly)
# ─────────────────────────────────────────────────────────────────────────────
...
def fr_single_step(
    ...
    """One step of Fruchterman-Reingold.
    ...
    temperature : float   current temperature (step-size cap)
    eps         : float   minimum distance clip (NetworkX uses 0.01)
    ...
    # repulsion: k²/d²  (all pairs)
    # attraction: A·d/k  (connected pairs only, unweighted)
...
def run_fr_with_snapshots(G: nx.Graph, n_iter: int = 50) -> Dict:
    """Run FR for n_iter steps, saving a snapshot at every step."""
    nodes = sorted(G.nodes())
    n     = len(nodes)

    # Unweighted adjacency matrix — weight=None ignores edge weights
    A   = nx.to_numpy_array(G, nodelist=nodes, dtype=np.float64, weight=None)
    pos = _grid_initial_positions(G)

    # FR hyper-parameters (identical to NetworkX)
    k  = np.sqrt(1.0 / n)
    coord_span = pos.max(axis=0) - pos.min(axis=0)
    t0 = float(np.max(coord_span) * 0.1)
    t0 = max(t0, 1e-4)
    dt = t0 / (n_iter + 1)          # linear cooling
...
def verify_against_networkx(G: nx.Graph, n_iter: int = 50) -> float:
    """Compare our final positions against NetworkX spring_layout."""
    nodes  = sorted(G.nodes())
    ...
    nx_final = np.array([nx_pos[node] for node in nodes], dtype=np.float32)
...
def main():
    ...
    if args.verify and all_graphs:
        ...
        print(f"  Max absolute deviation: {dev:.2e}")
