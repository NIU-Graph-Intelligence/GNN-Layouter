...
class AntiGCNConv(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        # Using an exponential function to increase repulsion strength
        repulsion_factor = torch.exp(self.anti_strength)
        anti_smooth_output = x_transformed - repulsion_factor * neighbor_mean
...
class GlobalRepulsionBranch(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_heads=8, dropout=0.1):
        ...
        # Add distance-aware projection
        self.distance_proj = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, x, edge_index, pos=None):
        ...
        h_attended, _ = self.global_attention(h_input, h_input, h_input)
        h_attended = h_attended.squeeze(0)

        # If current position estimates are available, use them to enhance repulsion
        if pos is not None:
            # Compute pairwise distances
            n_nodes = pos.size(0)
            pos_expanded_1 = pos.unsqueeze(1).expand(n_nodes, n_nodes, 2)
            pos_expanded_2 = pos.unsqueeze(0).expand(n_nodes, n_nodes, 2)
            dist_mat = torch.norm(pos_expanded_1 - pos_expanded_2, dim=2)

            # Convert distances to repulsion weights (closer = stronger repulsion)
            repulsion_weights = torch.exp(-dist_mat)
            repulsion_weights.fill_diagonal_(0)  # No self-repulsion

            # Apply repulsion
            repulsion = torch.matmul(repulsion_weights, self.distance_proj(h))
            h_global = h + h_attended - 0.1 * repulsion  # Subtract to create repulsion
        else:
            h_global = h + h_attended
